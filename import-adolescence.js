...
async function run() {
  ...
  const parsed = Papa.parse(content, {
      ...
      transformHeader: function(header, index) { 
          if (index === 0) {
              // PapaParse sometimes does multiple passes to guess delimiters.
              // We must reset the count every time it starts a new pass at column 0.
  ...
  const chapterTitle = 'Chapter 6: Adolescence - A Stage of Growth and Change';
  ...
  if (!targetChapter) {
      ...
      targetChapter = await Chapter.create({ subjectId: subject._id, title: chapterTitle, order: 6, isPublished: false });
  } else {
  ...
  for (let i = 0; i < rows.length; i++) {
     ...
     if (lessonTitle !== currentLessonTitle) {
         ...
         if (!titleCounts[lessonTitle]) {
             ...
             if ((/difficult module/i.test(lessonTitle)) && difficultBlocks > 1) {
                 // Format as 'difficult module 1' if there are multiple blocks
