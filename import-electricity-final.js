...
async function run() {
  ...
  console.log("🧹 Cleared old Electricity modules and revision questions for a fresh ordered upload.");
  ...
  for (const filePath of existingFiles) {
    ...
    const idxUnit = headers.findIndex(h => h.includes('unit_title') || h.includes('unit title') || h.includes('unit'));
    const idxLesson = headers.findIndex(h => h.includes('lesson_title') || h.includes('lesson title') || h.includes('lesson'));
    let idxType = headers.findIndex(h => h === 'type' || h.includes('type'));
    const idxConcept = headers.findIndex(h => h.includes('concept/statement') || h.includes('concept') || h.includes('statement'));
    ...
    const idxRevise = headers.findIndex(h => h.includes('revise') || h.includes('revis') || h === 'revision');
    ...
    for (let i = 1; i < rows.length; i++) {
       ...
       const reviseVal = (idxRevise !== -1 && row[idxRevise]) ? row[idxRevise].trim().toLowerCase() : '';
       // Add to revision if explicitly marked in CSV OR if the type is 'comic'
       if (reviseVal === 'y' || reviseVal === 'yes' || itemDoc.type === 'comic') {
  ...
  console.log("\n🎉 All CSVs successfully uploaded directly to the 'Electricity' chapter in CBSE Class 7!");
  process.exit(0);
}
