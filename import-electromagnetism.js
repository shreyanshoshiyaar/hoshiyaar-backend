...
async function run() {
  ...
  if (!targetChapter) {
      ...
      targetChapter = await Chapter.create({ subjectId: subject._id, title: chapterTitle, order: 4 });
  } else {
  ...
  for (let i = 0; i < rows.length; i++) {
     ...
     const images = [];
     ['image 1', 'image 2', 'image 3'].forEach(k => {
        if (row[k] && row[k].trim().startsWith('http')) {
