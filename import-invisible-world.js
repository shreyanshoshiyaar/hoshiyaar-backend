...
async function run() {
  ...
  const filePath = 'D:\\Invisible World - Final.csv';
  ...
  const chapterToClear = await Chapter.findOne({ title: 'Chapter 2: The Invisible Living World -  Beyond Our Naked Eye' });
  if (chapterToClear) {
      ...
      console.log("🧹 Cleared old Invisible World modules for a fresh ordered upload.");
  }
  ...
  const idxConcept = headers.findIndex(h => h === 'statement' || h === 'concept/statement');
  ...
  for (let i = 1; i < lines.length; i++) {
     ...
     const classTitle = row[idxClass] || '8';
     const subjectTitle = row[idxSubject] || 'Science';
     const chapterTitle = row[idxChapter] || 'Chapter 2: The Invisible Living World -  Beyond Our Naked Eye';
     ...
     if (!classLevel) classLevel = await ClassLevel.create({ boardId: board._id, name: classTitle, order: parseInt(classTitle) || 8 });
     ...
     if (!chapter) {
         // Auto calculate order based on current chapters in this subject
         const existingChaps = await Chapter.countDocuments({ subjectId: subject._id });
         chapter = await Chapter.create({ subjectId: subject._id, title: chapterTitle, order: existingChaps + 1 });
     }

     let unit = await Unit.findOne({ chapterId: chapter._id, title: unitTitle });
     if (!unit) {
        const existingUnits = await Unit.countDocuments({ chapterId: chapter._id });
        unit = await Unit.create({ chapterId: chapter._id, title: unitTitle, order: existingUnits + 1 });
     }
