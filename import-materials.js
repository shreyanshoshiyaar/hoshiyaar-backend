...
const DEFAULT_CSV = 'D:\\Materials around us final  (1).xlsx - Final Sheet Upload Akshit 7 sept.csv';
...
function parseOptions(optionsStr, answerText) {
  ...
  // If options contains semicolons, split by semicolon
  ...
  // If simple split matches the answer (case-insensitive) or no answer provided, return it
  if (!answerText || simpleOpts.some(o => o.toLowerCase() === answerText.trim().toLowerCase())) {
    return simpleOpts;
  }

  // Handle case where answer or options contain internal commas:
  // Check if answerText is inside optionsStr
  ...
  if (ansIdx !== -1) {
    // Extract answer and remaining parts before and after
    ...
    const result = [];
    if (before) {
      // Split before part if it has commas
      before.split(',').map(s => s.trim()).filter(Boolean).forEach(s => result.push(s));
    }
    result.push(ansTrim);
    if (after) {
      // Split after part if it has commas
      after.split(',').map(s => s.trim()).filter(Boolean).forEach(s => result.push(s));
    }
    return result;
  }

  // If opts length is 6 or 9, could be chunked
  if (simpleOpts.length === 6 || simpleOpts.length === 9) {
    const chunkSize = simpleOpts.length / 3;
    const chunked = [];
    for (let j = 0; j < simpleOpts.length; j += chunkSize) {
      chunked.push(simpleOpts.slice(j, j + chunkSize).join(', '));
    }
    if (chunked.some(o => o.toLowerCase() === lowerAns)) {
      return chunked;
    }
  }
...
async function run() {
  ...
  const chapterTitle = 'Chapter 6: Material Around Us';
  ...
  if (!isDryRun) {
    ...
    // Check for existing chapter: exact match or "Materials Around Us" placeholder
    targetChapter = await Chapter.findOne({
      subjectId: subject._id,
      $or: [
        { title: chapterTitle },
        { title: /Materials Around Us/i }
      ]
    });
    ...
    if (targetChapter) {
      ...
      targetChapter.isPublished = false; // Admins Only during/after import
      await targetChapter.save();
      console.log(`🔒 Updated title to "${chapterTitle}" & set to unpublished (Admins Only).`);
    } else {
      console.log(`⚠️ Creating new chapter "${chapterTitle}"...`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const conceptText = (row['statement'] || row['concept/statement'] || row['concept'] || '').trim();
    ...
    const lessonTitle = (row['lesson_title'] || row['lesson name'] || row['lesson title'] || row['lesson'] || '').trim();
    const unitTitleRaw = (row['unit_title'] || row['unit name'] || row['unit title'] || row['unit'] || 'Default Unit').trim();
    const reviseVal = (row['revise'] || row['revise?'] || '').trim().toLowerCase();
    const img1 = (row['image 1'] || row['images 1'] || '').trim();
    ...
    // Auto-fix FIB: Row 252 has answer in Options column
    if (mappedType === 'fib' && !answerText && optionsStr) {
      answerText = optionsStr;
    }

    // Auto-detect Rearrange questions labeled as MCQ
    if (mappedType === 'mcq' && /rearrange this sentence/i.test(questionText)) {
      mappedType = 'rearrange';
    }
    ...
    if (lessonTitle !== currentLessonTitle || (isDryRun && !currentModuleId)) {
      ...
      const finalModuleTitle = lessonTitle; // Preserve exact name including 'Difficult Module 1' / '2'
    ...
    imgHeaders.forEach(k => {
      if (row[k] && row[k].trim().startsWith('http') && !images.includes(row[k].trim())) {
