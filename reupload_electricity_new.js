...
const run = async () => {
  try {
    ...
    const chapter = await Chapter.findOne({ subjectId: subject._id, title: { $regex: 'Chapter 4: Electricity - Magnetic & Heating Effects', $options: 'i' } });
    if (!chapter) throw new Error("Chapter 4 not found");
