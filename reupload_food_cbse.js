...
const processCsv = async (csvPath, chapter, subject) => {
    ...
    for (const row of data) {
      const unitTitle = String(row['CBSE Unit Title'] || row.Unit_title || row.Unit || '').trim();
...
const run = async () => {
  try {
    ...
    const chapter = await Chapter.findOne({ title: { $regex: 'Chapter 3: Mindful eating', $options: 'i' } });
    if (!chapter) throw new Error("Chapter not found");

    const subject = await Subject.findById(chapter.subjectId);
    if (!subject) throw new Error("Subject not found");

    const csvPath = "D:\\Mindful eating - A path to a healthy body - CBSE - final - 5th June (1).csv";
