...
const run = async () => {
  try {
    ...
    const chapter = await Chapter.findOne({ title: { $regex: 'Chapter 4: The World of Metals', $options: 'i' } });
    if (!chapter) throw new Error("Chapter not found");

    const subject = await Subject.findById(chapter.subjectId);
    if (!subject) throw new Error("Subject not found");

    const csvPath = "D:\\\\World of Metals & Non-Metals - FInal Upload 20 June.csv";
