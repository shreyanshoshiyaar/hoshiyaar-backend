...
const processCsv = async (csvPath, chapter, subject) => {
    ...
    for (const [unitTitle, rows] of unitsMap.entries()) {
      ...
      let unit = await Unit.findOne({ chapterId: chapter._id, title: { $regex: new RegExp(`^${unitTitle.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i') } });
      
      if (!unit) {
         console.log(`Unit not found: ${unitTitle}. Creating it...`);
         ...
         // Clear existing modules and items for this unit if we are re-running
      ...
      for (const row of rows) {
        ...
        const imgKeys = Object.keys(row).filter(k => k.toLowerCase().includes('image') || k.toLowerCase().includes('image 1') || k.toLowerCase().includes('image 2') || k.toLowerCase().includes('image 3'));
        ...
        const reviseRaw = String(row['Revise?'] || row.Revise || row.revise || row['revise?'] || row.Revision || row.revision || '').trim().toLowerCase();
...
const run = async () => {
  try {
    ...
    // Strict lookups for exact Chapter target
    const board = await Board.findOne({ name: { $regex: 'CBSE', $options: 'i' } });
    if (!board) throw new Error("Board CBSE not found");
    
    const classLevel = await ClassLevel.findOne({ boardId: board._id, name: { $regex: '8', $options: 'i' } });
    if (!classLevel) throw new Error("ClassLevel 8 not found");
    
    const subject = await Subject.findOne({ classId: classLevel._id, name: { $regex: 'Science', $options: 'i' } });
    if (!subject) throw new Error("Subject Science not found");

    const chapterTitle = 'Chapter 4: Electricity - Magnetic & Heating Effects';
    let chapter = await Chapter.findOne({ subjectId: subject._id, title: { $regex: chapterTitle, $options: 'i' } });
    
    if (!chapter) {
        console.log(`Chapter not found. Creating new Chapter: ${chapterTitle}`);
        // Find highest order to place it at the end
        const existingChapters = await Chapter.find({ subjectId: subject._id }).sort({ order: -1 });
        const maxOrder = existingChapters.length > 0 ? existingChapters[0].order : 0;
    ...
    const csvPath = "D:\\\\Electromagnetism - Final - Akshit - 23 June.csv";
